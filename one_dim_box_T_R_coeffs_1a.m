...
%  region  II: psi_II(x)  = C*exp(i*k2*x) + D*exp(-i*k2*x)
...
%
function [] = one_dim_box_T_R_coeffs_1a
clc; format short
%
%E = 4.00;
V = 1.00; % E > V case is presented. 
%
N_energy = 1000;
delta_E = 0.01;
%
%%%%%%%%%%%%%%%%%%%%%%%%
fileID_save_data_1 = fopen('T_R_1d_test_2a.txt','w');
%
for ii = 1:N_energy
    %
    En = V + ii*delta_E;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
read_data = fopen('T_R_1d_test_2a.txt', 'r');               % 
...
%
plot(Energy(1:100:length(Energy)), T_coeff_exact(1:100:length(Energy)), 'ko','LineWidth', 1.5)
%
hold off
...
k2 = 2*mass*(E - V)/hbar2;
%
d11 = exp(ci.*k1.*aa); d12 = -exp(-ci.*k2*aa); d13 = -exp(ci.*k2*aa); d14 = 0.;
d21 = -k1*exp(ci.*k1*aa); d22 = -k2*exp(-ci*k2*aa); d23 = k2*exp(ci*k2*aa); d24 = 0.;
d31 = 0.; d32 = exp(ci*k2*aa); d33 = exp(-ci*k2*aa); d34 = -exp(ci*k1*aa);
d41 = 0.; d42 = k2*exp(ci*k2*aa); d43 = -k2*exp(-ci*k2*aa); d44 = -k1*exp(ci*k1*aa);
%
...
T_plus_R  = T + R;
%
T_exact = (1 + (1/4)*((k1^2-k2^2)/(k1*k2))^2 * sin(2*k2*aa)^2)^(-1);
%%%
return
end
